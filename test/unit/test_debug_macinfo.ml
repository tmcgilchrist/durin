...
let test_macinfo_define () =
  ...
  let section = Dwarf.parse_debug_macinfo_section cursor (List.length bytes) in
  ...
  check string "type is define" "DW_MACINFO_define"
    (Dwarf.string_of_macinfo_type entry.macinfo_type);
...
let test_macinfo_start_end_file () =
  ...
  check string "first is start_file" "DW_MACINFO_start_file"
    (Dwarf.string_of_macinfo_type first.macinfo_type);
  ...
  check string "second is end_file" "DW_MACINFO_end_file"
    (Dwarf.string_of_macinfo_type second.macinfo_type)
