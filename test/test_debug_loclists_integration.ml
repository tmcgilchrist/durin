...
let parse_loclists binary_path =
  let buffer = Object.Buffer.parse binary_path in
  Dwarf.DebugLoclists.parse buffer

let test_parse_succeeds binary_path =
  match parse_loclists binary_path with
  | None -> fail "expected DebugLoclists.parse to return Some"
  | Some _ -> ()
...
let test_resolve_first_list binary_path =
  ...
  | Some section ->
      if Array.length section.offset_table = 0 then
        ...
        let result =
          Dwarf.DebugLoclists.resolve_location_list buffer offset addr_size
        in
        check bool "resolve_location_list returns Some" true
...
let () =
  run_with_args "debug_loclists integration" binary_path
    [
      ...
      ( "resolve",
        [ ("resolve first location list", `Quick, test_resolve_first_list) ] );
    ]
