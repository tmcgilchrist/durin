...
let test_parse_succeeds binary_path =
  match Test_helpers.find_section binary_path ".debug_frame" with
  ...
  | Some (buffer, section) ->
      ...
      let parsed = Dwarf.CallFrame.parse_debug_frame_section cur size in
      check bool "entry_count > 0" true (parsed.entry_count > 0)
