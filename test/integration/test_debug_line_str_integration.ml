...
let parse_line_str binary_path =
  let buffer = Object.Buffer.parse binary_path in
  Dwarf.DebugLineStr.parse buffer

let test_parse_succeeds binary_path =
  match parse_line_str binary_path with
  | None -> fail "expected DebugLineStr.parse to return Some"
  | Some _ -> ()
      ...
      let has_source_file =
        Array.exists
          (fun (e : Dwarf.DebugLineStr.string_entry) ->
            String.equal e.content "hello_world.c")
          t.entries
      in
      check bool "contains \"hello_world.c\"" true has_source_file
...
let () =
  run_with_args "debug_line_str integration" binary_path
    [
